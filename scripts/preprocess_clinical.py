...
def process_and_save_folds(
    input_csv, output_dir, n_splits=5, id_column="patient_id", label_column="label"
):
    """
    ...
    patient_ids = (
        ...
        else pd.Series(np.arange(len(df)), name=id_column)
    )
    ...
    for fold, (train_idx, val_idx) in enumerate(kf.split(X, y)):
        ...
        print(
            f"  SMOTE 过采样完成, 训练样本从 {len(X_train)} 增加到 {len(X_train_resampled)}"
        )
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="临床数据预处理脚本，用于交叉验证，可防止数据泄漏。"
    )
    parser.add_argument("input_csv", help="原始临床数据 CSV 文件路径")
    parser.add_argument("output_dir", help="保存处理后各折数据文件的目录")
    parser.add_argument(
        "--n_splits", type=int, default=5, help="交叉验证的折数，默认为 5"
    )
    parser.add_argument(
        "--id_column", default="patient_id", help="患者ID列名，此列不作为特征"
    )
