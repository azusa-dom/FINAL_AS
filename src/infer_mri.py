...
def parse_args():
    ...
    parser.add_argument(
        "--model_path", type=str, required=True, help="模型参数路径，如 best_fold1.pth"
    )
    parser.add_argument(
        "--class_names", nargs="+", required=True, help="类别名列表，如 0_Healthy 1_AS"
    )
    parser.add_argument(
        "--device", type=str, default="cuda" if torch.cuda.is_available() else "cpu"
    )
    parser.add_argument(
        "--save_dir", type=str, default="cam_output", help="Grad-CAM 保存目录"
    )
...
def main():
    ...
    out_path = os.path.join(
        args.save_dir, f"{os.path.basename(args.img_path).split('.')[0]}_cam.jpg"
    )
