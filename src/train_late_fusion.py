...
def load_and_merge_predictions(
    clinical_preds_dir, mri_preds_dir, n_folds=5, clinical_target_index=1
):
    """
    ...
    for i in range(n_folds):
        try:
            ...
            if PATIENT_ID_COL not in df_clin.columns:
                raise ValueError(
                    f"临床预测文件 {clin_path} 中缺少 '{PATIENT_ID_COL}' 列。"
                )
            ...
            if PATIENT_ID_COL not in df_mri.columns:
                raise ValueError(
                    f"MRI预测文件 {mri_path} 中缺少 '{PATIENT_ID_COL}' 列。"
                )

            df_mri = df_mri.rename(
                columns={"preds": "mri_score", "labels": "true_label"}
            )
            ...
            print(
                f"  ✅ 成功加载并合并 Fold {i} 的数据。合并后样本数: {len(df_merged)}"
            )
    ...
    if initial_rows > final_rows:
        print(
            f"⚠️ 警告: 在数据清理中丢弃了 {initial_rows - final_rows} 行，因为它们包含NaN值。"
        )
...
def main(args):
    fusion_df = load_and_merge_predictions(
        args.clinical_preds_dir,
        args.mri_preds_dir,
        args.n_splits,
        clinical_target_index=args.clinical_target_index,
    )
    ...
    for fold, (train_idx, test_idx) in enumerate(skf.split(X, y)):
        ...
        print(
            f"  Fold {fold+1} | AUC: {metrics['auc'][-1]:.4f}, Acc: {metrics['accuracy'][-1]:.4f}"
        )

    print("\n========== Fusion CV Summary ==========")
    for name, values in metrics.items():
        print(
            f"  {name.capitalize():<10}: {np.mean(values):.4f} ± {np.std(values):.4f}"
        )
    print("=======================================\n")
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--clinical_preds_dir", type=str, default="models/clinical_model/clinical_preds"
    )
    parser.add_argument(
        "--mri_preds_dir", type=str, default="models/mri_model/mri_preds"
    )
    ...
    parser.add_argument(
        "--clinical_target_index", type=int, default=1, help="用于融合的logit类索引"
    )
