...
class CNSStyle:
    ...
    # --- 专业色阶 ---
    @classmethod
    def create_cmap(cls, name='cns_thermal'):
        """创建符合出版要求的渐变色阶"""
        gradients = {
        ...
        return LinearSegmentedColormap.from_list(name, gradients[name])
...
def configure_cns_style(debug=False):
    ...
    # The register_cmap calls are now moved outside this function to ensure they are called only once per session.
    # So, these lines should NOT be in configure_cns_style anymore:
    # plt.register_cmap(cmap=CNSStyle.create_cmap('cns_thermal'))
    # plt.register_cmap(cmap=CNSStyle.create_cmap('cns_diverging'))
...
# We use it to apply settings and register colormaps ONCE.
if __name__ == '__main__':
    configure_cns_style(debug=True) # Apply main theme settings
    print("CNS Visualization Theme initialized successfully (when run directly)!")
    
    # Register custom colormaps ONCE, whether run directly or imported.
    # Check if a colormap is already registered to prevent ValueError if module is somehow reloaded
    if 'cns_thermal' not in plt.colormaps():
        plt.register_cmap(cmap=CNSStyle.create_cmap('cns_thermal'))
    if 'cns_diverging' not in plt.colormaps():
        plt.register_cmap(cmap=CNSStyle.create_cmap('cns_diverging'))

else: # This block executes when theme.py is imported by another script.
    # Apply standard theme settings (non-debug) automatically upon import.
    configure_cns_style(debug=False)
    
    # Register custom colormaps ONCE when the module is first loaded via import.
    # This prevents the "already registered" ValueError on subsequent calls to configure_cns_style.
    if 'cns_thermal' not in plt.colormaps():
        plt.register_cmap(cmap=CNSStyle.create_cmap('cns_thermal'))
    if 'cns_diverging' not in plt.colormaps():
        plt.register_cmap(cmap=CNSStyle.create_cmap('cns_diverging'))