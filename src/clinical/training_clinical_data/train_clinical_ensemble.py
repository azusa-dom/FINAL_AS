...
def main():
    ...
    for fold_idx, (train_idx, val_idx) in enumerate(skf.split(X, y)):
        ...
        # 1. ClinicalNet (Gradient Boosting) - Main model
        print("Training ClinicalNet (Gradient Boosting)...")
        clinical_net = ClinicalNet(n_estimators=200, learning_rate=0.05, max_depth=6, random_state=42)
        clinical_net.fit(X_train_scaled, y_train)
        models['clinical_net'] = clinical_net
        ...
        # ClinicalNet predictions (main model)
        clinical_net_prob = clinical_net.predict_proba(X_val_scaled)[:, 1]
        predictions['clinical_net'] = clinical_net_prob
        ...
        for i, (idx, true_label) in enumerate(zip(val_idx, y_val)):
            all_predictions.append({
                ...
                'fold': fold_idx,
                'clinical_net_prob': predictions['clinical_net'][i],
        ...
        if args.save_models:
            ...
            # Save sklearn models
            joblib.dump(clinical_net.model, os.path.join(fold_dir, 'clinical_net_model.pkl'))
    ...
    print("\n=== Overall Results ===")
    overall_metrics = {}
    for model in ['clinical_net', 'lightgbm', 'xgboost', 'neural_network', 'logistic_regression', 'ensemble']:
